...
from friedman2 import *
import friedman_config as c

import matplotlib.pyplot as plt

X, y = friedman2(n_samples=300, add_noise=True)
X_altered, y_altered = friedman2_altered(n_samples=300, d=0.1, add_noise=True)
plt.plot(X[:, 2], y, '.')
...
for seed in c.seed_list:
    ...
    for d in c.d_list:
        ...
        X_target_test, y_target_test = friedman2(
            n_samples=1000,
            add_noise=False,
            noise_distribution='gaussian',
            random_seed=seed)  #do NOT add noise to test set!!!!
        X_target_val, y_target_val = friedman2(
            n_samples=1000,
            add_noise=False,
            noise_distribution='gaussian',
            random_seed=seed + 10)  #do NOT add noise to test set!!!!
        X_target_train, y_target_train = friedman2(
            n_samples=200,
            add_noise=True,
            noise_distribution='gaussian',
            random_seed=seed)  #add noise to train set
        X_source_train, y_source_train = friedman2_altered(
            n_samples=1000,
            add_noise=True,
            noise_distribution='gaussian',
            d=d,
        ...
        for config in c.param_grid_LSTransferTreeBoost:
            ...
            df.to_csv('results/200_gaussian.csv')
