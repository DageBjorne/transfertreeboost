...
ablation_transfer_real = pd.DataFrame(columns=[
    ...
    'source_tree_size', 'target_tree_size', 'k', 'm_0', 'val_rmse', 'val_mae',
    'rmse', 'mae'
])
...
def train_run_tester(s_idx):
    ...
    #             X_source_train_with_dummy = np.hstack(
    #                 (X_source_train.copy(), zeros))
    ...
    #                     f'results/xgboost_ablation_rs.csv')
    ...
    #                 ablation_transfer_real = pd.DataFrame(columns=[
    #                     'seed', 'target_column', 'target_instances', 'method',
    #                     'v', 'target_tree_size', 'val_rmse', 'val_mae', 'rmse',
    #                     'mae'
    #                 ])
    #                 ablation_transfer_real = pd.DataFrame(columns=[
    #                     'seed', 'target_column', 'target_instances', 'method',
    ...
    #                     (X_target_train_with_dummy, X_source_train_with_dummy))
    #                 y_comb = np.concatenate((y_target_train, y_source_train))
    #                 bst = train_xgboost(X_comb,
    #                                     y_comb,
    #                                     X_target_val_with_dummy,
    #                                     y_target_val,
    #                                     boosting_rounds=1000,
    #                                     params=params)
    ...
    #                     target_tree_size, val_rmse, val_mae, rmse, mae
    #                 ]
    #                 ablation_file = f'results/xgboost_ablation_rs.csv'
    #                 file_exists = os.path.isfile(ablation_file)
    #                 ablation_transfer_real.to_csv(ablation_file,
    #                                               mode='a',
    #                                               header=not file_exists)
...
ablation_transfer_real = pd.DataFrame(columns=[
    'seed', 'target_column', 'target_instances', 'method', 'base_lr',
    'fine_tuning_lr', 'dropout_rate', 'batch_norm', 'val_rmse', 'val_mae',
    'rmse', 'mae'
])
                        ...
                        for batch_norm in include_batch_norm:
                            ...
                            ablation_transfer_real.loc[len(
                                ablation_transfer_real)] = [
                                    ...
                                    batch_norm, val_rmse, val_mae, rmse, mae
                                ]
