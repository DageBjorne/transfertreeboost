...
for seed in c.seed_list:
    ...
    for d in c.d_list:
        ...
        X_source_train, X_source_val = train_test_split(X_source, test_size=0.2, random_state=seed)
        y_source_train, y_source_val = train_test_split(y_source, test_size=0.2, random_state=seed)
        ...
        for config in c.param_grid_XGBoost:
            ...
            df.to_csv(f'results_300/xgb_warmstart.csv') #change save_folder depending on split
