...
for seed in c.seed_list:
    ...
    X_target_test_, y_target_test = friedman1(
        n_samples=1000,
        add_noise=False,
        noise_distribution='gaussian',
        n_features=10,
        random_seed=seed)  #do NOT add noise to test set!!!!
    X_target_val_, y_target_val = friedman1(
        n_samples=1000,
        add_noise=False,
        noise_distribution='gaussian',
        n_features=10,
        random_seed=seed + 10)  #do NOT add noise to test set!!!!
    X_target_train_, y_target_train = friedman1(
        n_samples=300,
        add_noise=False,
        noise_distribution='gaussian',
        n_features=10,
    ...
    for d in c.d_list:
        ...
        # Source = 0
        source_indicator = np.zeros((X_source_train_.shape[0], 1))
        X_source_train = np.hstack((X_source_train_, source_indicator))

        # Target = 1
        target_indicator = np.ones((X_target_train_.shape[0], 1))
        X_target_train = np.hstack((X_target_train_, target_indicator))
        ...
        val_indicator = np.ones((X_target_val_.shape[0], 1))
        X_target_val = np.hstack((X_target_val_, val_indicator))

        # Test set (target → 1)
        test_indicator = np.ones((X_target_test_.shape[0], 1))
        X_target_test = np.hstack((X_target_test_, test_indicator))
        ...
        for config in c.param_grid_XGBoost:
            ...
            df.to_csv(f'results_300/xgb_naive.csv')
